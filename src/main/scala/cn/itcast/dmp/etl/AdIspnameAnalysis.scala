...
//todo:广告投放的运营商分布情况统计
object AdIspnameAnalysis  extends ProcessData{
  ...
  val sinkTable="adIspnameAnalysis"
  ...
  override def process(sparkSession: SparkSession): Unit = {
    ...
    val adIspnameDF: DataFrame = sparkSession.sql(ContantsSQL.adIspnameSQL1)

    adIspnameDF.createOrReplaceTempView("adIspname")
    val result: DataFrame = sparkSession.sql(ContantsSQL.adIspnameSQL2)

    //3、保存结果数据到kudu表中
    val schema: Schema = ContantsSchema.adIspnameSchema
